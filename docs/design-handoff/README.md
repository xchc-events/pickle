...
>
> Some things count as today's however far off the night: a part that wants attention or is blocked, artwork up for sign-off, a booking still moving, a night to settle. Other unfinished work on a confirmed booking counts once the night is inside the Pipeline's "Next 30 days", or if a coordinator has flagged it. Once a night has passed its departments are history and only settling it is, except that a booking which never reached confirmed is still asked about, since nothing else would say so. The list shows the six most pressing and counts the rest: anything blocked first, then whatever is due soonest (the booking's own target for the booking, the door for everything else).
>
...
>
> It is one form with two readers. The venue's people fill in the whole booking: the night, who is bringing it (the venue itself, an organisation on file, or a name not on file yet), its owner, the booking model, the acts with a status and a fee floor and ceiling, the split, the attendance spread and the cost inputs. Those start from the prototype's own `mk()` defaults, shown in the form for a coordinator to change, never applied silently. Ticket prices are left to Ticketing, which already owns them. They can hold the room in the same step. An outside promoter fills in the night they would like, the room, the kind of night, the acts' names and a note. The server builds the rest of their enquiry from fixed values whatever the request carries: their own organisation, the date as a preference (TBC) until a coordinator locks it, no owner, no figures, no hold. Nothing a promoter types reaches a figure on the P&L. See `cleanEnquiry` in `src/lib/intake.ts`.
>
