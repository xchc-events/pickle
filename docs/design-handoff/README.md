...
> **Changed 23 Sep 2026 — shifts offered to you (R6).** "Other times they're assigning people and hoping they take those ones, and they should be able to come in and confirm them" (Connor). A person with a User now sees their own offered shifts in a list of its own, **"Shifts offered to you"**, sitting above Needs you — each row a role, the event and when, with Confirm and Decline buttons rather than a link out, since this is the one thing on Home answered here rather than fixed somewhere else. Gated on nothing but being the person the shift is offered to: the query itself is scoped to the reader's own `personId` and `state: 'OFFERED'`, so another account cannot reach a shift by guessing its id. Deliberately not part of the parts/gates machinery "Needs you" and "Nobody's on these" are built from (`needsFor` in `src/lib/home.ts`) — an offered shift is personal, not a department's, so it is `loadHome`'s own query rather than a `Need`. Confirming books the hour entry at the reader's own rate, the same transaction Roster's "Confirm now" and the emailed link at `/s/[token]` both use. `confirmMyOffer`/`declineMyOffer` in `src/app/(app)/home/actions.ts`; see "Roster" above for the rest of R6.
...
> **Changed 23 Sep 2026 — codes and the door list.** Connor: "You should also have a field for discount codes, and completely free ticket codes. Also access codes: codes that allow you to access certain ticket types. You should be able to send people a code to open and access only that ticket type." · "Door lists as well should be able to be entered in this section." Built in Pickle now, below the tiers table, and pushed to Gather.rsvp once its codes and door APIs exist (`docs/gather-rsvp-api.md`, sections 5 and 6) — the venue is not waiting on that client to stop keeping this on paper. A **Codes** section: a `TicketCode` per event — percent off, amount off, free, or one that unlocks a tier not otherwise on sale — with a state (active, scheduled, expired, used up, off), an add form and deactivate; `uses` is Gather's redemption count, read back once that client exists, so a code changes nothing in the projection before then. A **Door list** section: a `DoorListEntry` per name, grouped by kind (guest, comp, industry, act) with a running total, an add form and remove, and a plain, printable view at `/ticketing/door-list?event=…` for the door itself, grouped the same way with a blank box per row for a pen tick. Both are venue only — gated on the ticketing module like the rest of this page, but refused for an external account the way `setOrganisation` refuses one in Admin, since these are the venue's own levers, not a promoter's, even on their own event. **The door list now feeds the settlement**: the comps P&L line (`financeVals`'s `comps`, src/lib/finance.ts, untouched) reads the list's COMP entries, party sizes summed, once any exist for the event, in place of the typed `crew` figure it read before — `compsCountFor` in `src/lib/door-list.ts`, proved against the real `financeVals` in its test. PG-11's guest list now exists in the product; per-act allocation, a cutoff, door check-in reconciling arrivals, and acts submitting their own list through the `AccessGrant` token flow remain open.
...
> **Changed 23 Sep 2026 — a shift can be renamed, retimed, duplicated and deleted.** "I need the ability to actually manipulate these shifts, rather than just the person on them. It would be great to duplicate a shift or delete a shift, so we can duplicate a number of set-up crew for example, and to change the title of these things" (Connor). Each shift row now carries a small editor: the role name, and its start and end as clock times on the night, derived from doors and the offset and written back as the offset and hours the plan and the P&L read — `renameShift` touches only the role, `retimeShift` moves the linked hour entry's hours with it in one transaction. Duplicate opens a copy with no person and no hour entry; delete takes its hour entry with it, unless that entry is already paid, in which case it refuses. Pack-in and pack-out show as the night's bounds, but a shift may still run outside them — the night is not a hard rule. `renameShift`, `retimeShift`, `duplicateShift` and `deleteShift` in `src/app/(app)/roster/actions.ts`.

> **Changed 23 Sep 2026 — the roster shows the sales.** "It would be helpful on the rostering page to see the current ticket sales figure and the projected likely ticket sales, so when people are rostering it's possible to see whether we're likely to need everyone who is rostered, or if we might be cancelling some shifts" (Connor). A strip beside the two headline figures now reads sold so far, the likely-turnout scenario the event is staffed to, the room's capacity, and how many shifts the standard plan wants at that turnout against how many exist. Read-only, from exactly the sources named for it — `readSales`, `Event.att[1]`, `capacityOf` and `shiftPlan` — composed in `salesStripFor` in `src/lib/roster-data.ts`.

> **Changed 23 Sep 2026 — a shift is offered, then confirmed (R6).** "When you select someone, before they're confirmed, have them as nominated, then confirmed. Click their little symbol to view their contact information so you can call or text, or push a button to send an email alert that they've been offered a shift. Sometimes the person will be chatting with the crew member and chucking them on right then; other times they're assigning people and hoping they take those ones, and they should be able to come in and confirm them" (Connor). Picking somebody in the roster no longer books the hours on the spot: the shift moves to a new `OFFERED` state, person set, nothing booked yet. From there the duty manager can **Confirm now** (they said yes in the room), **Decline** (they said no) or **Email offer** — a `ShiftOffer` row good for one reply, on the `AccessGrant` pattern (32 random bytes, only the SHA-256 kept), expiring in 7 days. Confirming — however it happens — books the hour entry exactly as picking used to. Rosterable crew can be either accounts or people on file, Connor decided, so both confirm paths exist: a person with no User answers through the emailed link at `/s/[token]`, no sign-in needed; one with a User can also confirm or decline from their own Home, under "Shifts offered to you". The avatar on a shift row is now a button: it opens a popover with the person's email and phone as selectable text, and "Email the offer" while one is live. `offerShift`, `confirmOffer`, `declineOffer`, `emailOffer` in `src/app/(app)/roster/actions.ts`; the confirm/decline transaction all three paths share is `confirmOfferedShift`/`declineOfferedShift` in `src/lib/shift-offers-data.ts`.

> **Changed 23 Sep 2026 — the night as a timeline, with drag.** "To see in a more visual way what period of time each person is going to be on site." "It would be nice to just click and drag to move the beginning and end times of those shifts around, something very similar to the Google Calendar day or week view." "It should pull through the pack-in time and pack-out time to the roster, so you can really clearly see that" (Connor). Above the shift list, an hour axis now runs from an hour before pack-in (or doors) to an hour after pack-out (or everyone out), with the five run-time marks on it — pack-in, doors, bar close, everyone out, pack-out — and one bar a shift, the person's initials on it, open shifts in the warning tone. Dragging a bar's ends — pointer, or the arrow keys on a focused handle — snaps to 15 minutes and calls the same `retimeShift` the shift editor's own times already do; dragging the middle moves the whole shift, same length. The list underneath is unchanged and stays the way to edit on a phone. The axis, each bar's geometry as fractions, and the snap are a pure, tested function in `src/lib/roster-timeline.ts`; `RosterTimeline.tsx` only draws what it computes.
